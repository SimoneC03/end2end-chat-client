  ...
  <div class="friends h-screen bg-gray-900">
    ...
    <div :class="{'friends_list w-11/12 md:w-5/12 lg:w-3/12 mx-auto':true, 'lg:flex lg:flex-row lg:w-screen justify-center':user.friends.filter(item => item.confirmed==true).length>1}" v-if="user.friends.filter(item => item.confirmed==true).length>0">
      <div v-for="(el, index) in user.friends.filter(item => item.confirmed==true)" :key="index" :class="{'lg:w-12/12':true,'lg:w-3/12': user.friends.filter(item => item.confirmed==true).length>1}">
        <div :class="{'friend bg-gray-700 hover:bg-gray-600 flex flex-row p-6 my-4 shadow-lg rounded-lg cursor-pointer text-gray-200': true, 'lg:w-12/12 mx-3':user.friends.filter(item => item.confirmed==true).length>1}" v-if="el.confirmed == true" @click="$router.push({name: 'Chat', params: {username: el.username}})">
          <div class="friend_img rounded-full w-20 h-20 bg-gray-900" :style="'background-image:url(https://avatars.dicebear.com/api/gridy/'+el.username+'.svg)'"></div>
    ...
    <div class="w-11/12 md:w-7/12 lg:w-5/12 mx-auto flex flex-row">
      <input type="text" placeholder="Insert the username who want to invite" class="bg-gray-700 shadow p-3 m-1 focus:outline-none rounded w-10/12 text-gray-200" v-model="friendToInvite">
      <button class="p-3 bg-green-400 block rounded my-3 w-2/12 ml-2" :disabled="inviteLoading" @click="inviteFriend"><span v-show="!inviteLoading">Invite</span><i class="fa fa-spinner" v-show="inviteLoading"></i></button>
    </div>
    ...
    <div :class="{'invites_list w-11/12 md:w-5/12 lg:w-3/12 mx-auto':true, 'lg:flex lg:flex-row':user.friends.filter(item => item.confirmed==false).length>1}" v-if="user.friends.filter(item => item.confirmed==false).length>0">
      <div v-for="(el, index) in user.friends.filter(item => item.confirmed==false)" :key="index" class="w-12/12">
        <div :class="{'invite bg-gray-600 hover:bg-gray-700 flex flex-row p-6 my-4 shadow-lg rounded-lg items-center w-12/12': true, 'lg:w-5/12': user.friends.filter(item => (item.confirmed==false && hasInvited==false)).length>1}" v-if="el.confirmed === false && el.hasInvited === false">
          <div class="invite_img rounded-full w-20 h-20 bg-gray-900 bg-cover" :style="'background-image:url(https://avatars.dicebear.com/api/gridy/'+el.username+'.svg)'"></div>
